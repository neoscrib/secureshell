...
{
    class DHGProcessor : DiffieHellmanProcessor
    {
        private byte[] p;
        private byte[] g = new byte[] { 0x02 };
        private byte[] ClientPrivateKey;
        private byte[] ServerPublicKey;
        private byte[] ClientPublicKey;

        public DHGProcessor(Session session)
            : base(session)
        {
            p = GetP();
        }
        ...
        {
            ClientPublicKey = CalculateE(p, g);
            var geRequest = new SshDHKexInit(ClientPublicKey);
        ...
        internal byte[] GetP()
        {
            if (session.Algorithms.KexAlgorithms[0] == "diffie-hellman-group1-sha1")
                return new byte[] {
            ...
            else if (session.Algorithms.KexAlgorithms[0] == "diffie-hellman-group14-sha1")
                return new byte[] {
                    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xc9, 0x0f, 0xda, 0xa2, 0x21, 0x68, 0xc2, 0x34, 
                    0xc4, 0xc6, 0x62, 0x8b, 0x80, 0xdc, 0x1c, 0xd1, 0x29, 0x02, 0x4e, 0x08, 0x8a, 0x67, 0xcc, 0x74, 
                    0x02, 0x0b, 0xbe, 0xa6, 0x3b, 0x13, 0x9b, 0x22, 0x51, 0x4a, 0x08, 0x79, 0x8e, 0x34, 0x04, 0xdd, 
                ...
                throw new CryptographicException("Unsupported group exchange identifier.");
        }
    }
}
